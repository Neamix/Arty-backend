...
function makeStage(): ProjectStage
{
...
function makeLead(ProjectStage $stage, int $sortOrder): ProjectLead
{
    return ProjectLead::create([
...
it('spaces appended leads by the sort gap', function () {
    $stage = makeStage();
    $repo = app(ProjectLeadRepository::class);
...
it('averages neighbours when dropping between two cards', function () {
    ...
    app(ProjectLeadService::class)->move($moved, $stage->id, $first->id, $second->id);
...
it('halves the first order when dropping at the top', function () {
    ...
    app(ProjectLeadService::class)->move($moved, $stage->id, null, $first->id);
...
it('appends past the last order when dropping at the bottom', function () {
    ...
    app(ProjectLeadService::class)->move($moved, $stage->id, $last->id, null);
...
it('rebalances the stage when the neighbour gap is too small', function () {
    ...
    $orders = ProjectLead::where('project_stage_id', $stage->id)
        ->pluck('sort_order');
...
it('moves a lead into another stage between its neighbours', function () {
    ...
    app(ProjectLeadService::class)->move($moved, $target->id, $a->id, $b->id);

    expect($moved->fresh()->project_stage_id)->toBe($target->id);
    expect($moved->fresh()->sort_order)->toBe(150000);
});
